...
dt =  0.1
potential_planets = []
force_planets = []  #lists for tese values are initilised ere and empty so thhat in every itertion, thhe values calculated can be added for the four planets
potenergy_planets = []
radius_orbit = []
timeperiod_planets = []
...
#the list is sperated into the valies calculated for each planet an dthhus different significant plots are made for each planets, energy, potential, force acting on it and radial distance from sun
